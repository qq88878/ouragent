...
CREATE TABLE IF NOT EXISTS `learning_path` (
    ...
    `version`        INT           NOT NULL DEFAULT 1 COMMENT '路径版本号',
    `archived`       TINYINT       NOT NULL DEFAULT 0 COMMENT '0=正常 1=已归档',
    `starred`        TINYINT       NOT NULL DEFAULT 0 COMMENT '0=未收藏 1=已收藏',
    `create_time`    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `update_time`    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`        TINYINT       NOT NULL DEFAULT 0,
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `learning_path_step` (
    ...
    `step_type`      VARCHAR(32)   DEFAULT NULL COMMENT 'CONCEPT/PRACTICE/REVIEW/PROJECT',
    `estimated_hours` INT          DEFAULT NULL COMMENT '预估耗时(小时)',
    `create_time`    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `update_time`    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`        TINYINT       NOT NULL DEFAULT 0,
    PRIMARY KEY (`id`),

...
async function doUpload() { if (!uploadFile.value) return; uploading.value = true; try { const r = await knowledgeApi.upload(uploadFile.value); if (r.code === 200) { ElMessage.success('上传成功'); uploadFile.value = null; await loadCourseMaterials(materialsCourse.value.id); } else { ElMessage.error(r.message || '上传失败'); } } catch { ElMessage.error('上传失败'); } finally { uploading.value = false; } }
